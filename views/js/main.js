...
var pizzaElementGenerator = function(i) {
  ...
  /*Using a css class instead of explicitly setting the width and the height*/
...
/*
  This is a new resizePizzas function. Since we already know that
  pizza image has to be scaled to 25%, 33.33%, and 50% on small, medium and large
  selections respectively, these css classes are created to scale the width to
  the desired percentages. So, when a size is selected using the slider, required
  css class is added to the ".randomPizzaContainer".
*/
var resizePizzas = function(size){
  window.performance.mark("mark_start_resize");   // User Timing API function

  function resizeAllPizzas(className){
    var allPizzas = document.querySelectorAll(".randomPizzaContainer");
    for (var i = 0; i < allPizzas.length; i++) {
      allPizzas[i].className = "randomPizzaContainer "+className;
    }
  }
  switch(size){
    case "1":
       document.querySelector("#pizzaSize").innerHTML = "Small";
       resizeAllPizzas("small");
       break;
    case "2":
       document.querySelector("#pizzaSize").innerHTML = "Medium";
       resizeAllPizzas("medium");
       break;
    case "3":
       document.querySelector("#pizzaSize").innerHTML = "Large";
       resizeAllPizzas("large");
       break;
    default:
...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
    //var dx = determineDx(document.querySelectorAll(".randomPizzaContainer")[0], size); //Added
